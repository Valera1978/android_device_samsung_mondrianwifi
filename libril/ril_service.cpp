    ...
    if (radioService[slotId]->mRadioResponse != NULL) {
        ...
        if (response == NULL) {
            ...
            if ((numStrings != dataRegRespStrings) && (numStrings != 11)) {
                RLOGE("getDataRegistrationStateResponse Invalid response: NULL (numStrings(%d) != %d/11)", numStrings, dataRegRespStrings);
                if (e == RIL_E_SUCCESS) responseInfo.error = RadioError::INVALID_RESPONSE;
            } else {
                char **resp = (char **) response;
...
int radio::getOperatorResponse(int slotId,
    ...
    if (radioService[slotId]->mRadioResponse != NULL) {
        ...
        hidl_string numeric;
        int numStrings = responseLen / sizeof(char *);
        if (response == NULL || ((numStrings != 3) && (numStrings != mqanelements - 2))) {
            RLOGE("getOperatorResponse Invalid response: numStrings(%d) != (%d and %d)",numStrings,3,mqanelements - 2);
            if (e == RIL_E_SUCCESS) responseInfo.error = RadioError::INVALID_RESPONSE;
        } else {
            char **resp = (char **) response;
            longName = convertCharPtrToHidlString(resp[0]);
            shortName = numStrings == 3 ? convertCharPtrToHidlString(resp[0]) : convertCharPtrToHidlString(resp[0]);
